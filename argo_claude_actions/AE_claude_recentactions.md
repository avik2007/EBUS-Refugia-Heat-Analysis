...
## 2026-09-19 — Direction change: spatial moving-window GP; AWS/Coiled closed out; git cleanup started

- User asked why no spatially local GP was tried. Record: never considered in specs; d_0 became a per-window
  optimizer output (unidentified). User wants d_0 physical (undercurrent vs CCS / coast vs gyre turbulence).
- Read Kuusela & Stein 2018 (Proc R Soc A 474:20180400) from Zotero PDF via `uv run --with pypdf`: stationary GP
  fit by ML in spatial (20 deg) + temporal (1/3-month) moving window, exponential kernel, years as replicates,
  Student-t nugget via Laplace (unstable in places). Decision: stay Gaussian, keep our domain.
- Fact found: `GibbsKernel` spatial part is squared-exponential though its header comment says Matern-0.5-like.
  Gibbs is a signal kernel, not a noise kernel (noise = `WhiteKernel`).
- Deferred by user (nothing launched, no S3 prefix, no Coiled job): AWS optimizer multi-start test; 32 remaining LML sweeps.
- Plan approved: `.claude/plans/we-need-to-look-mutable-rabbit.md` (Part A git cleanup, Part C pilot).

---

## 2026-09-18 — d_0 identifiability study: 2010-2020 ingest + Gibbs re-run + d_0 distribution plots + first LML sweep

**Headline: the d_0 scatter is not a 2015 fluke.** Across 33 layer-years (11 years x 3 layers, 378 rolling
45-day windows per layer) the CV of fitted `d_transition_km` is 0.57-0.76 pooled and 0.21-1.14 within single
layer-years. Many windows sit on an optimiser bound.

| Layer | pooled CV | per-year CV range | windows at lower bound (50 km) | windows at upper bound |
|---|---|---|---|---|
| Skin (0-100 m) | 0.76 | 0.21-1.08 | 22% | 15% (700 km) |
| Source (150-400 m) | 0.69 | 0.24-1.14 | 25% | 38% (700 km) |
| Background (500-1000 m) | 0.57 | 0.32-0.96 | 9% | 36% (1500 km) |

CV convention: sample std (ddof=1). 2015 therefore reads 0.70/0.78/0.61 (Skin/Source/Background), slightly above
the 0.67/0.74/0.61 logged 2026-09-17 (population std) — same data, different convention.

### Where the plots / outputs are
- **Figure** (per-year boxes with each year's CV printed above the box; pooled histograms per layer with pooled CV,
  median, % pinned at bounds; dashed lines = optimiser bounds):
  `AEResults/aeplots/vertical_delta/d0_distribution_2010_2020.png`
- **Stats table** (per layer-year plus a pooled `all` row: n_windows, mean, median, std, CV, frac at lower/upper bound):
  `AEResults/aelogs/d0_distribution_multiyear_stats.csv`
- Generated by `d0_distribution_multiyear.py` (repo root). Palette: categorical slots 1-3 (blue #2a78d6 Skin, orange
  #eb6834 Source, aqua #1baf7a Background) run through the dataviz skill's `validate_palette.js` — all checks PASS;
  aqua contrast vs surface 2.74:1 (WARN) is covered by direct CV labels + the stats CSV. Three render-and-look
  iterations fixed label collisions (CV label vs y tick, stats text over the bound spike, bound labels spilling
  between panels).
- **Deviation from approved plan step 7:** built this as a new script; `analyze_d0_distribution()` in
  `vertical_delta_analysis.py` left untouched (its 2015 plot/CSV unchanged); one multi-year figure instead of
  separate per-year PNGs.
- Sweep output: `AEResults/aelogs/d0_lml_sweep/d0_lml_sweep_d150_400_2015.csv` (204 rows = 34 windows x 6 d_0 values).
- Per-layer-year Gibbs audits: `AEResults/aelogs/californiav3_<year>0101_<year>1231_res0_5x0_5_t10_0_<layer>_3dgibbs_w45_timelsfix[_d1500]_lml/audit_*.csv`
  (33 folders; 500-1000 m uses `_timelsfix_d1500_lml`); each also has a sibling manifest folder without `_3dgibbs_w45`.

### 1. Bug found and fixed: any non-2015 config silently ran on 2015 data
- Root cause: `run_ingestion_pipeline` (`ArgoEBUSCloud/02_ae_cloud_run.py`) dropped `date_start/date_end`;
  `get_ae_config` then used the registry default for `californiav3` (2015-01-01..2015-12-31, `ebus_core/ae_utils.py:40`);
  `run_diagnostic_inspection` (`05_ae_update_tomatern0.5.py`) swallowed the dates via `**_`; `ebus_core/runner.py`
  `run_analysis` never passed them. A 2012 config would have re-ingested 2015 while run_id/manifest claimed 2012.
  Existing 2015 outputs are unaffected (they were 2015 anyway).
- Fix: `run_cloud_pipeline` takes `start_date/end_date` and passes them to `get_ae_config`; `run_ingestion_pipeline`
  forwards ISO strings; script 05 takes `date_start/date_end` -> `get_ae_config`; `runner.run_analysis` adds
  `date_start/date_end` to `dispatch_kwargs`.
- Tests written first (4 failing, then passing): `test_run_analysis_dispatch_kwargs_include_dates`,
  `test_run_ingestion_pipeline_forwards_dates`, `test_run_cloud_pipeline_passes_dates_to_get_ae_config`,
  `test_run_diagnostic_inspection_passes_dates_to_get_ae_config` (+ helper `_load_script`).

### 2. LML now recorded per window
`record['lml'] = gp.log_marginal_likelihood_value_` added to the common per-window record in
`analyze_rolling_correlations` (`ebus_core/argoebus_gp_physics.py`; all kernel types; the value belongs to the final
post-auto-calibration GP). Test: `test_analyze_rolling_correlations_records_lml`. Suite total:
`test_mlops_foundation.py` + `test_thermodynamics.py` = 96 passed. Gotcha: the CLI tests use repo-root-relative paths, so
run pytest from the repo root — from inside `ArgoEBUSCloud/` 5 CLI tests fail spuriously (cwd artifact).

### 3. Ingest 2010-2020 (AWS/Coiled), 33 parquets in `s3://argo-ebus-project-data-abm/`
- Probe first: 2010 d0_100, 4m19s (Coiled cluster, 3 spot workers, us-east-1, ~4 min of that is cluster startup).
  982 rows / 41 floats vs 2015's 1388 / 55; `time_bin` offset 1830 d from 2015 (1826 d + 10 d bin alignment) — confirms
  it is really 2010 data. Manifest recorded `s3_path: null` (pre-existing: `run_ingestion_pipeline` returns `{}`).
- Remaining 29 (2015 skipped; I first said 32 — miscount, corrected) as 3 parallel per-layer chains: 24 ok first pass,
  5 failed (2011 d0_100, 2014 d0_100, 2014 d500_1000, 2012 d150_400, 2020 d150_400) with
  `unpack requires a buffer of N bytes` (Coiled/Dask comms error; N = 105712/416/50592/448). Retried all 5 in parallel:
  4 ok, d500_1000 2014 failed again and left a **partial** write (8 of 12 parts, 437 rows, 24 floats); rerun alone
  succeeded (12 parts, 730 rows, 32 floats). Nothing was deleted from S3 (checked stale files by part timestamps).
- Validation (`check_parquets` script, all 33): 12 parts each, one write timestamp per parquet (no stale leftovers),
  contiguous `time_bin` 4010-8030, correct year per file. Deep-layer (500-1000 m) rows per year 2010-2020
  range 693-1349 (2012 lowest at 693; 2014 = 730 after the rerun; 2015 = 1349). (An earlier chat message wrongly called 2014
  the lowest.)
- Configs: 30 new `configs/californiav3/californiav3_<YYYYMMDD>_<YYYYMMDD>_res0_5x0_5_t10_0_<layer>_ingest.yaml`
  (copied from the 2015 templates, only dates changed). `AEResults/run_registry.jsonl` = 82 lines, all parse as JSON
  (checked at session end, after both the 3 parallel ingest chains and the 3 parallel analysis chains appended).

### 4. Gibbs re-run, 33 layer-years, local
- Configs `configs/californiav3/californiav3_<year>_<layer>_gibbs_lml.yaml` (33, from the `_timelsfix` templates;
  500-1000 m keeps the 1500 km d_0 bound), `run_suffix` `_timelsfix_lml` / `_timelsfix_d1500_lml` so existing 2015
  outputs are untouched, `outputs.generate_snapshots/physics_plots: false` to save time. Time-ls bounds 15-200 d,
  spatial upper bound 10, 45 d window, 10 d stride.
- Probe 2010 Source: 1m49s (~9.5 min CPU, multi-core; d_0 CV 1.14). Remaining 32 as 3 parallel per-layer chains on the
  16-core box (deep-layer runs 6-10 min each); all 33 exit 0.

### 5. LML sweep (`d0_lml_sweep.py`, repo root) — 2015 Source only
Clamps d_0 on a 6-point grid (50/100/200/400/800/1500 km, bounds d0 x (1 -/+ 5e-4); equal bounds avoided because sklearn
treats them as a fixed hyperparameter) and refits every window through `analyze_rolling_correlations` so windows,
calibration and other hyperparameters are identical to the free fits.
- Clamp check: max relative deviation of returned d_0 from the requested value = 0.05% (= the band). 204 rows, no NaN.
- LML swing across the grid per window: median 9.8 nats (mean 15.1, min 0.01, max 61.9). Median LML by d_0
  (50..1500): -78.6, -78.2, -74.4, -74.3, -77.7, -74.4 (bumpy, non-monotone). Best-d_0 counts over 34 windows
  (50/100/200/400/800/1500): 4/2/4/10/8/6 (14/34 prefer >=800 km, above Source's 700 km free-fit bound).
- Free fit vs clamped: free LML minus grid-max LML: median 0.00, min -39.8, max +12.1; in 12/34 windows a clamped fit
  beats the free fit by >1 nat (up to 39.8) => the free-fit optimizer misses better solutions in those windows. 8/34
  free-fit windows sit on a bound.
- Interpretation: NOT cleanly flat and NOT peaked; consistent with (a) weak/ambiguous identification and (b)
  optimizer failure. Caveats: one layer-year; auto-calibration re-tunes noise after the fit so the LML compared is
  the calibrated model's; the sweep's runtime was lost with the scratchpad log (cost of a full sweep unmeasured).
- Plain-language explanation given to user: nats = natural-log likelihood ratio (1 nat ~ 3x, 10 nats ~ 22,000x);
  free fit = optimizer picks d_0; clamped fit = d_0 forced to one value; "0.05%" = clamp sanity check; 4/2/4/10/8/6 =
  per-window argmax counts.
- **Next test proposed (NOT run):** free refits from 6 starting d_0 values on AWS (Coiled batch) — now the [TOP] todo
  item, awaiting approval. Nothing was launched on AWS for it.

### 6. Corrections to things I said or that the docs say
- User caught: I re-proposed the GibbsKernel test suite from a stale todo item ("DIDN'T WE ALREADY DO THE GIBBS KERNEL
  TESTS?"). They exist (13 tests, `test_mlops_foundation.py:1381-1563`); todo item annotated, lesson #7 added.
- I said `AE_gemini_todo.md` still asserted the d_0 values as "AUDITED & STATISTICALLY PROVEN": wrong — the d_0 bullet
  was already retracted there on 2026-09-17 (only the header wording remains). Added an update bullet with the new evidence.
- Antigravity's 2026-09-18 log states "Claude's 2026-09-17 finding of flat marginal-likelihood surfaces in d_0" — that
  was an untested hypothesis, and the sweep does not cleanly support it (see #5). Flagged in `AE_gemini_todo.md` note.
- Also: I said the sweep was "still running" when it had already finished (I could not see buffered `conda run` output).

### 7. Left open / not done
- Remaining 32 layer-year sweeps; LML-vs-d_0 curve plots (plan step 6 output not produced); constant/pooled d_0 decision;
  the AWS optimizer test (awaiting approval); `s3_path: null` manifest gap; ingest-failure root cause; signature-drift test.
- **Nothing committed; no branch created.** Everything above is uncommitted on `fix/restore-antigravity-hln-vertical-delta`,
  alongside the earlier held changes (`californiav3_d500_1000_gibbs_timelsfix.yaml`, `vertical_delta_analysis.py`).
- Process: plans `.claude/plans/let-s-do-2010-2020-ingest-majestic-kite.md` (approved, executed steps 1-5, part of 6-7)
  and `.claude/plans/let-s-make-revioewing-this-merry-ritchie.md` (docs plan). The dual-graph MCP `graph_continue` was
  not called this session (policy miss); `CONTEXT.md` created at session end.

---

## 2026-09-16 — Antigravity handoff closed: HLN DM test + vertical delta script reviewed and run

Both items from the top-of-todo `[HANDOFF FROM ANTIGRAVITY]` entry executed and verified:
- `compare_kernels.py` (HLN correction): ran clean, output matches Antigravity's
  2026-09-16 review numbers exactly. Gibbs statistically superior on all 3 layers
  under HLN small-sample correction, including Source (p=0.0374, the tightest one,
  survives the conservative correction).
- `vertical_delta_analysis.py`: ran clean, produced
  `AEResults/aelogs/vertical_delta_californiav3_2015.csv` + 3 synthesis plots.
  Flagged one caveat before trusting the output: Background layer's `d_transition_km`
  median landed exactly on the 700km config bound — led directly into the
  identifiability investigation below.

Answers the old Priority 3 "Vertical Delta Comparison Script" key question
("is Source warming faster than Background?") is deferred pending the d_0
finding below — RMSRE/anisotropy numbers are trustworthy, `d_0` numbers are not.

---

## 2026-09-17 — d_transition_km (d_0) identifiability failure found across all 3 layers

### What happened
Widened Background layer's `d_transition_bounds_km` upper bound 700→1500km (user
decision, motivated by session 17/19's flagged 700km saturation) in
`configs/californiav3/californiav3_d500_1000_gibbs_timelsfix.yaml`, new run_suffix
`_timelsfix_d1500`, grounded the new bound by computing the californiav3 domain's
actual max `dist_to_coast` directly (`calculate_dist_to_coast` over the
lat[30,48]/lon[-135,-115] box: max 1404.8km, p95 1029km, median 380km) so 1500km
sits just past the domain's true physical extent rather than being an arbitrary
round number. Re-ran (`aebus_cli.py analyze`, ~9 min). Background median d_0 moved
off the bound to 965.8km (mean 888.9km) — looked like a real fitted value.

### Follow-up check (user request) — red flag, all 3 layers
Added `analyze_d0_distribution()` to `vertical_delta_analysis.py`: per-layer
boxplot + jittered per-window scatter of `d_transition_km`
(`AEResults/aeplots/vertical_delta/vertical_delta_d0_distribution.png`) plus a
stats table (`AEResults/aelogs/vertical_delta_d0_distribution_stats.csv`) with
mean/std/coefficient-of-variation and fraction of windows pinned at each
config bound.

**Result: coefficient of variation 0.6–0.74 in all three layers** (Skin 0.67,
Source 0.74, Background 0.61) — window-to-window `d_0` scatter is on the order
of ±60-75% of the mean, not a tight physical scale. Background still has 29% of
windows pinned at the new 1500km ceiling even after widening. None of the three
layers pass a sane identifiability bar (flagged all 3 as "not reliably
identified" in the diagnostic's printed verdict).

### Implication
The vertical fingerprint narrative built on median `d_0` per layer (Skin ~204km,
Source ~227km/290km, Background ~700-966km, "coastal-confined upper layers vs.
basin-scale deep layer") is **not currently a trustworthy claim** — the medians
are summarizing a distribution wide enough that the summary statistic itself
isn't meaningful. `AE_gemini_todo.md` line 17 (session 20, "AUDITED &
STATISTICALLY PROVEN") already asserts this as confirmed; it is not corrected in
that file pending science review (see new `[For Antigravity]` item). Anisotropy
ratio does not show this problem and remains the more defensible per-layer
metric for now.

### Likely cause (untested)
Single-window CV fits (34 windows/layer, ~45-day span each) probably lack enough
spatial contrast in `dist_to_coast` per window to pin down a sigmoid inflection
point — the log-likelihood surface in `d_0` is plausibly near-flat, so the
optimizer lands near its init or a bound rather than a genuine optimum. Not
verified this session (would need a likelihood-surface sweep in `d_0`, same
style as the `time_ls` sensitivity sweep from session 19's units-bug
investigation).

### Files changed
- `configs/californiav3/californiav3_d500_1000_gibbs_timelsfix.yaml` — bound
  widened + provenance comment.
- `vertical_delta_analysis.py` — background folder pointer updated to
  `_timelsfix_d1500` run; added `analyze_d0_distribution()` + wired into
  `run_vertical_delta_analysis()`; fixed a matplotlib `boxplot(labels=...)`
  deprecation warning in the new code.
- New run outputs (gitignored, not committed): `AEResults/aelogs/californiav3_..._d500_1000_..._timelsfix_d1500/`,
...
- Config/script changes not yet committed — holding per user instruction pending
  the identifiability question above.

### Next steps (see top of `AE_claude_todo.md` and new `AE_gemini_todo.md` item)
- Gemini/Antigravity science review requested: is `d_0` salvageable as a
  per-window metric, or should the vertical-fingerprint claim rest on anisotropy
  ratio alone?
- If pursuing a fix: likelihood-surface sweep in `d_0` (same method as the
  `time_ls` sweep) to confirm the flat-likelihood hypothesis before trying pooled
  fitting or a tighter init.

---

## 2026-09-16 — Todo hygiene: closed out stale Priority 1/2 (californiav2/FX2), ticked Background Blob case study, consolidated Vertical Delta duplicate

### Priority 1 (FX2/californiav2 diagnosis) — retroactively closed, all 3 items answered by later sessions
- Source Layer regression root cause: domain-clipping in californiav2; fixed by californiav3 (8.13% → 3.05% RMSRE, session 11, 2026-05-03).
- `scale_time_bin` saturating at 45d (Skin/Source): resolved as a window-design limit, not a bounds issue — confirmed unresolvable even at a 200d ceiling (session 19, 2026-07-17); report `time_ls_days` as "≥200d, unresolvable at 45-day window" going forward, not a widen-the-bound fix.
- Background Z=18.73 spike (window 6102.5, ~Sep 2015): confirmed Pacific Blob onset, consistent with californiav3 Background spikes (11.35/9.28/9.11, session 11).

### Priority 2 — last open item ticked
- Background window 5955–6000 (May 2015) case study: Z>2.0 persists at fine (t10) temporal resolution — confirms Blob onset is a genuine stationarity violation, not a coarse-binning artifact (californiav3 session 11). Gibbs kernel (sessions 16–19) is what actually resolves the calibration blowup this caused (Background Std_Z 1.72±2.63 → 0.97–1.00).

### Priority 3 — item 1 consolidated, not separately tracked
"Vertical Delta Comparison Script" was a duplicate of the top-of-file Antigravity handoff item (`vertical_delta_analysis.py`, written 2026-08-30, committed `765ec96` 2026-09-16, still pending review/run). Removed the duplicate bottom-of-file listing; its key question ("is Source warming faster than Background?") folded into the handoff entry note. Not marked done — script exists but hasn't been executed yet.

**Todo trimmed:** removed Priority 1 and Priority 2 sections entirely from `AE_claude_todo.md`; Priority 3 now lists only the two genuinely open items (Seasonal Anisotropy Report, SST Cross-Validation).
